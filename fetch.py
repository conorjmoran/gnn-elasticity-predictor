...
def fetch_and_build(out_dir: str, limit: int, nn_method: str, cutoff: float, api_key: Optional[str],
    ...
    saved = 0
    skipped = 0
    iterable = docs if fetch_all else docs[:limit]
...
def main():
    ...
    p.add_argument("--limit", type=int, default=20, help="Number of materials to fetch (ignored with --all)")
    ...
    p.add_argument("--all", action="store_true", help="Fetch all matching materials (server-side pagination)")
    p.add_argument("--page-size", type=int, default=1000, help="Page size when --all is used")
    try:
        bool_action = argparse.BooleanOptionalAction
    except AttributeError:
        bool_action = 'store_true'
    if isinstance(bool_action, type):
        p.add_argument("--skip-existing", action=bool_action, default=True,
                       help="Skip writing graphs that already exist (resume support)")
        p.add_argument("--quiet", action=bool_action, default=False,
                       help="Suppress common benign warnings (CrystalNN radii, Pauling EN)")
    else:
        p.add_argument("--skip-existing", action=bool_action, default=False,
                       help="Skip writing graphs that already exist (resume support)")
        p.add_argument("--quiet", action=bool_action, default=False,
                       help="Suppress common benign warnings (CrystalNN radii, Pauling EN)")
    ...
    try:
        ...
        # Fallback shim for older argparse: provide only enabling flag
        bool_action = 'store_true'
    if isinstance(bool_action, type):
        p.add_argument("--guess-oxidation-states", action=bool_action, default=True,
                       help="Guess oxidation states before neighbor finding (improves CrystalNN)")
    else:
        p.add_argument("--guess-oxidation-states", action=bool_action, default=False,
                       help="Guess oxidation states before neighbor finding (improves CrystalNN)")

    default_mat2vec = Path(__file__).with_name("mat2vec_embeddings.json")
    ...
    fetch_and_build(
        ...
        guess_oxidation=getattr(args, 'guess_oxidation_states', True),
        fetch_all=args.all,
