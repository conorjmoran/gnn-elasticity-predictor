...
try:
    from scripts.train import PtGraphDataset, LogTransformer, AlignnRegressor, compute_error_stats
except ModuleNotFoundError:
    from train import PtGraphDataset, LogTransformer, AlignnRegressor, compute_error_stats  # type: ignore

DEFAULT_MIN_LOGVAR_FLOOR = -2.9
try:
    from scripts.train_2 import HeteroAlignnRegressor, apply_conformal_intervals, MIN_LOGVAR_FLOOR as DEFAULT_MIN_LOGVAR_FLOOR  # type: ignore
except ModuleNotFoundError:
    try:
        from train_2 import HeteroAlignnRegressor, apply_conformal_intervals, MIN_LOGVAR_FLOOR as DEFAULT_MIN_LOGVAR_FLOOR  # type: ignore
    except ModuleNotFoundError:
        try:
            from scripts.train_ensemble import HeteroAlignnRegressor, apply_conformal_intervals
        except ModuleNotFoundError:
            from train_ensemble import HeteroAlignnRegressor, apply_conformal_intervals  # type: ignore
...
def reliability_curve_from_conformal(
    s_scores: torch.Tensor, coverages: List[float]
) -> Tuple[List[float], List[float]]:
    """Compute reliability using calibration conformity scores s = |y - mu|/sigma or |y-mu| (already in z-space)."""
    nom, emp = [], []
    n = s_scores.size(0)
    for p in coverages:
        alpha = 1.0 - p
        q_level = min(max(math.ceil((n + 1) * (1 - alpha)) / n, 0.0), 1.0)
        q = torch.quantile(s_scores, q_level, dim=0)
        # By construction, coverage is p in calibration; on test it is empirical
        # We compute expected (nominal) p and empirical via test coverage when applied.
        nom.append(p)
        emp.append(float("nan"))
    return nom, emp
...
def main() -> None:
    ...
    corr_out = ens_dir / f"corr_heatmap_{split_tag}.png"
    ...
    plot_parity(targets_np, mean_np, target_names, ens_dir / f"parity_{split_tag}.png")
    plot_residuals(mean_np, residuals_np, target_names, ens_dir / f"residuals_vs_pred_{split_tag}.png")
    if emp_g.size > 0:
        plot_reliability(nom_g, emp_g, target_names, ens_dir / f"reliability_gaussian_{split_tag}.png", "Reliability (Gaussian)")
    if sharp_widths_arr.size > 0:
        plot_sharpness_coverage(sharp_widths_arr, sharp_covers_arr, target_names, ens_dir / f"sharpness_vs_coverage_{split_tag}.png")
    # Error-variance plot
    se2 = (targets_z - mean_z).pow(2).detach().cpu().numpy()
    if std_z is not None:
        var_np = std_z.pow(2).detach().cpu().numpy()
        plot_error_variance(se2, var_np, target_names, ens_dir / f"error_variance_{split_tag}.png")
    ...
    # Save metrics JSON
    with open(ens_dir / f"eval_metrics_{split_tag}.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    print(f"Saved ensemble evaluation for validation split:")
    print(f"  Metrics -> {ens_dir / ('eval_metrics_' + split_tag + '.json')}")
    print(f"  Parity -> {ens_dir / ('parity_' + split_tag + '.png')}")
    print(f"  Residuals -> {ens_dir / ('residuals_vs_pred_' + split_tag + '.png')}")
    rel_path = ens_dir / ("reliability_gaussian_" + split_tag + ".png")
    if rel_path.exists():
        print(f"  Reliability (Gaussian) -> {rel_path}")
    sharp_path = ens_dir / ("sharpness_vs_coverage_" + split_tag + ".png")
    if sharp_path.exists():
        print(f"  Sharpness vs Coverage -> {sharp_path}")
    if corr_out.exists():
        print(f"  Correlation heatmap -> {corr_out}")
    print(f"  Error-Variance -> {ens_dir / ('error_variance_' + split_tag + '.png')}")
